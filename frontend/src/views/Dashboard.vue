...
import { listStudents, getMonitorStatus, logout, clearToken, toggleMonitorStudent } from '../api'
...
async function onToggleMonitor(s: Student) {
  if (togglingId.value) return
  togglingId.value = s.student_id
  try {
    const updated = await toggleMonitorStudent(s.student_id)
    const i = students.value.findIndex(x => x.student_id === s.student_id)
    if (i >= 0) students.value[i] = updated
    getMonitorStatus().then(st => { status.value = st }).catch(() => {})
  } catch (e: any) {
    alert(e.response?.data?.detail || '切换监控失败')
  } finally {
    togglingId.value = ''
  }
}
    ...
    <header class="topbar">
      <div class="topbar-inner">
        <div class="brand">
          ...
          <h1>RUC Helper · 管理台</h1>
        </div>

        <div class="controls">
          <button class="btn-icon" @click="doLogout" title="退出登录">
    ...
    <main class="main">
      ...
      <div v-else class="roster">
        <StudentCard v-for="s in students" :key="s.student_id" :student="s"
                     :busy="togglingId === s.student_id" @toggle-monitor="onToggleMonitor" />
      </div>
    </main>
